...
\end{equation}
Like before, we can multiply by ${\U\rho}_i$ and trace to obtain the Pauli-decomposition, and use the cyclic trace rule, and reorder the terms:
\begin{align*}
  \C{U}_i
  &= 
    \frac{1}{4}
...
\begin{align*}
  \C{K}_{ij}^{(1)}
  &= 
  \frac{1}{4} \tr\left\{ \big[{\U \rho_i}, {\U\Sigma}^{(1)}\big] {\U G}^R {\U \rho}_j - {\U G}^A \big[{\U \rho_i}, {\U\Sigma}^{(1)}\big] {\U \rho}_j \right\},
  \\
  \C{K}_{ij}^{(2)}
  &= 
  \frac{1}{4} \tr 
...
This yields an explicit linear differential equation for $\C{H}$, which can be solved very efficiently numerically.
